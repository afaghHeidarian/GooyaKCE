...
export class PublicVarService {
 ...
 WMTSTrafficLayerName = 'VwTraffic';
 //  WMTSTrafficLayerName = 'traffic_view';
 ...
 //  styleCheck = { styleEn: 'Auto', styleFa: 'اتوماتیک' };
 styleMode = 'Auto';
 ...
 WMTSLayerDAYFAPOI = new TileLayer({
  ...
  source: new WMTS({
   ...
   tileGrid: new WMTSTileGrid({
    origin: [ this.projectionExtent[0], this.projectionExtent[3] ],
 ...
 WMTSLayerTerrain = new TileLayer({
  ...
  source: new WMTS({
   ...
   tileGrid: new WMTSTileGrid({
    ...
    extent: [ 5152078.29, 3452021.1, 6638063.34, 4591795.67 ],
   }),
   style: '',
   wrapX: true,
   tileLoadFunction: this.tileLoader,
 ...
 WMTSLayerOddEvenArea = new TileLayer({
  ...
  source: new WMTS({
   ...
   tileGrid: new WMTSTileGrid({
    ...
    matrixIds: this.matrixId,
    // az geoserver begirim extent ro
    extent: [ 5016353.97, 4251739.56, 5731325.03, 4517514.12 ],
   }),
   style: '',
   wrapX: true,
   tileLoadFunction: this.tileLoader,
 ...
 //     'services/Demographics/USA_Population_Density/MapServer/">ArcGIS</s>',
 //    url: this.WMTSUrl,
 //    layer: this.WMTSTrafficLayerName,
 //    matrixSet: this.projLike,
 //    format: 'image/png',
 //    projection: this.projections,
 //    tileGrid: new WMTSTileGrid({
 ...
 WMTSLayerTraffic = new TileLayer({
  source: new TileWMS({
   url: 'http://45.82.138.85:6672/geoserver/Gooya2018Q3_V2_New/wms',
   ...
   params: {
    ...
    // style: 'Gooya2018Q3_V2_New:TRAFFIC'
   },
   serverType: 'geoserver',
   transition: 0,
   //  tileLoadFunction: this.tileLoaderWMS,
  }),
  zIndex: 4,
 });
 ...
 //   format: new GeoJSON(),
 //   url: function(extent) {
 //     return 'http://45.82.138.85:6672/geoserver/wfs?service=WFS&' +
 //         'version=1.1.0&request=GetFeature&typename=Gooya2018Q3_V2_New:traffic&' +
 ...
 // WMTSLayerTraffic = new VectorLayer({
 ...
 SatelliteOverlayFA = new TileLayer({
  ...
  source: new WMTS({
   ...
   tileGrid: new WMTSTileGrid({
    ...
    matrixIds: this.matrixId,
    // az geoserver begirim extent ro
    extent: [ 4902831.14, 2884211.38, 20037508.34, 4833345.25 ],
   }),
 ...
 tileLoaderWMS(tile, src) {
  console.log(tile.tileCoord[0]);
  // for limited zoom less than 11 and dont send request
  tile.getImage().src = src;

  // setInterval(e => {
  //  tile.getImage().src = src;
  // }, 10000);
  // if (tile.tileCoord[0] >= 11) {
